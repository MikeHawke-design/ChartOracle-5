...
import { AnalysisResults, StrategyKey, UploadedImageKeys, User, Trade, UserSettings, SavedTrade, UserUsage, StrategyLogicData } from '../types.ts';
...
const AnalysisView: React.FC<AnalysisViewProps> = ({ 
    ...
    const getDisplayTrades = (category: keyof AnalysisResults): Trade[] => {
        ...
        const originalTradesForCategory = analysisResults[category] || [];
        const modifiedTradesForCategory = modifiedAnalysis ? (modifiedAnalysis[category] || []) : [];

        if (modifiedTradesForCategory.length === 0) return originalTradesForCategory;

        const modifiedTradesMap = new Map<string, Trade>();
        modifiedTradesForCategory.forEach((modTrade: Trade) => {
            const key = `${modTrade.symbol}:${modTrade.direction}:${modTrade.type}`;
            modifiedTradesMap.set(key, { ...modTrade, isModified: true });
        });

        return originalTradesForCategory.map((origTrade: Trade) => {
    ...
    const normalizePrice = (priceString: string | number): string => {
        // Returns a string with fixed precision to avoid floating point comparison issues.
    ...
    const isTradeSaved = (trade: Trade): boolean => {
        ...
        // Don't match if any price is invalid.
    ...
    const redoButtonText = "Redo Analysis";
        ...
        <div className="text-center">
            ...
            <p className="text-gray-400" style={{ fontSize: `${userSettings.uiFontSize}px` }}>Using <span className="font-semibold text-yellow-400">
                {selectedStrategies.length > 1 
                    ? `Confluent (${selectedStrategies.map((s: StrategyKey) => strategyLogicData[s]?.name || s).join(' + ')})` 
                    : strategyLogicData[selectedStrategies[0]]?.name || selectedStrategies[0]}
            </span> Strategy</p>
        </div>
        ...
        {uploadedImages && Object.keys(uploadedImages).length > 0 && (
            <div className="my-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 p-4 bg-gray-800/50 rounded-lg">
                     {Object.keys(uploadedImages).map((key: string) => {
                        const index = parseInt(key, 10);
                        const imageSrc = uploadedImages[index as keyof typeof uploadedImages];
                        const step = TIME_FRAMES_STEPS.find(s => s.step === index + 1); // Use find to be safe
                        if (imageSrc && step) {
                            return (
                                <AnalyzedChartImage key={step.step} step={step} imageSrc={imageSrc} />
                            );
                        }
                        return null;
                    })}
