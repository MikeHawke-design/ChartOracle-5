...
const App: React.FC = () => {
    ...
    const handleAuthSuccess = () => {
        ...
        if (!apiConfig.geminiApiKey && !apiConfig.openaiApiKey) {
            ...
            {!isAuthenticated ? (
                ...
                <>
                    ...
                    <ApiKeyOnboardingModal 
                        ...
                        onSave={(keys) => {
                            setApiConfig((prev: ApiConfiguration) => ({...prev, geminiApiKey: keys.gemini, openaiApiKey: keys.openai}));
                            
                            const hasGemini = !!keys.gemini;
                            const hasOpenAI = !!keys.openai;
                    
                            // Automatically set provider if only one key is given
                            if (hasGemini && !hasOpenAI) {
                                setUserSettings(prev => ({ ...prev, aiProvider: 'gemini' }));
                            } else if (!hasGemini && hasOpenAI) {
                                setUserSettings(prev => ({ ...prev, aiProvider: 'openai' }));
                            }
                            // If both are provided, the default 'gemini' from DEFAULT_USER_SETTINGS will be used initially.
